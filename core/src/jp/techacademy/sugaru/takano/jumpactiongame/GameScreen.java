...
public class GameScreen extends ScreenAdapter {
    ...
    public GameScreen(JumpActionGame game){
        ...
        mEnemys = new ArrayList<Enemy>();//課題
        ...
        mPrefs = Gdx.app.getPreferences("jp.techacademy.sugaru.takano.jumpactiongame");
        mHighScore = mPrefs.getInteger("HIGHSCORE",0);

        enemyHit = Gdx.audio.newSound(Gdx.files.internal("enemyhit.mp3"));
        starHit = Gdx.audio.newSound(Gdx.files.internal("starhit.mp3"));
        bgm = Gdx.audio.newMusic(Gdx.files.internal("jagbgm.mp3"));
    ...
    private void createStage() {
        ...
        while (y < WORLD_HEIGHT - 5) {
            ...
            if (mRandom.nextFloat() > 0.8f){
                ...
                float xx = mRandom.nextFloat() * (WORLD_WIDTH - Enemy.ENEMY_WIDTH);
                float yy = mRandom.nextFloat() * (WORLD_HEIGHT - 5 - Enemy.ENEMY_HEIGHT) + Enemy.ENEMY_HEIGHT;
    ...
    private void checkCollision() {
        ...
        //Enemyとの当たり判定
        for (int a = 0;a < mEnemys.size(); a++){
            Enemy enemy = mEnemys.get(a);
